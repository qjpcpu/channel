...
func newScheduler(j *pipeImpl) *scheduler {
...
func (s *scheduler) waitRead() {
	...
	// drop data by filter
	if s.pipeImpl.filter != nil && !s.pipeImpl.filter(dataVal) {
		return
	}
	s.pipeImpl.queueSize++
...
func (s *scheduler) tryReadOrWrite() (chosen int, recv reflect.Value, ok bool) {
	if buff := atomic.LoadUint64(&s.pipeImpl.maxIn); s.pipeImpl.queueSize >= buff {
		// block read channel
		s.readAndWriteChannels[readI].Chan = s.dummyC
		chosen, recv, ok = reflect.Select(s.readAndWriteChannels)
		// restore readC
		s.readAndWriteChannels[readI].Chan = s.pipeImpl.readC
	} else {
		chosen, recv, ok = reflect.Select(s.readAndWriteChannels)
	}
	return
...
func (s *scheduler) prepareNextWrite() {
	for s.pipeImpl.queueSize > 0 {
		val, _ := s.pipeImpl.list.pop()
		if s.pipeImpl.filter != nil && !s.pipeImpl.filter(val.Interface()) {
			s.pipeImpl.queueSize--
		} else {
